...
private:
    ...
    void printSurnamesAtLeafsRecursively(Node<T> *node);
...
template<typename T>
int BinaryTree<T>::getNodesCount(){
    return nodesCount;
}
...
template<typename T>
void BinaryTree<T>::printSurnamesAtLeafsRecursively(Node<T> *node) {
    if (node == nullptr) return;

    if (node->left == nullptr && node->right == nullptr){
        Student st = node->data;
        cout<<"Leaf node student's surname: "<<st.getSurname()<<endl;
    } else {
        printSurnamesAtLeafsRecursively(node->left);
        printSurnamesAtLeafsRecursively(node->right);
    }
}

template<typename T>
void BinaryTree<T>::printSurnamesAtLeafs(){
    printSurnamesAtLeafsRecursively(root);
}
...
void DisplayMainMenu(){
    ...
    cout << "7 - Print surnames at leafs\n";
    cout << "0 - Quit\n";
