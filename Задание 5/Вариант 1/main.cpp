...
private:
    ...
    void countNodesExceptLeafsRecursively(Node<T> *root);
...
template<typename T>
void BinaryTree<T>::countNodesExceptLeafsRecursively(BinaryTree::Node<T> *node) {
    if (node == nullptr) return;


    if (node->left || node->right){
        countNodesExceptLeafsRecursively(node->left);
        countNodesExceptLeafsRecursively(node->right);
        nodesCount++;
    }
}

template<typename T>
void BinaryTree<T>::countNodesExceptLeafs(){
    countNodesExceptLeafsRecursively(root);
    cout<<"Nodes count: "<<getNodesCount()<<endl;
}
