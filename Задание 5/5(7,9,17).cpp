...
void node_max(const node_t* tr, int& max, int& count) {
  int m, c;
  max=(tr->data.history+tr->data.physics+tr->data.math)/3;
  count=1;
  if (tr->left){
    node_max(tr->left, m,c);
    if (m>max){
        max=m;
        count=c;
    }
