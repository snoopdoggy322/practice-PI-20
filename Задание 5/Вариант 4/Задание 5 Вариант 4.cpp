...
Node* findHelper(Node* root,Node* result){
	if(root!=NULL) 
	printf("%d",root->data.id);
	if(root!=NULL) {
		if(root->data.math==5&&root->data.history==5&&root->data.physics==5)
			insert(&result, root->data);
		findHelper(root->left,result);
		findHelper(root->right,result);
	}
	return result;
}
Node* findBest(Node* root) {
	Node *Result = NULL;
	Result = findHelper(root, Result);
	return Result;
}
...
struct Node * executeAction(int choice,Node * root) {
	...
	switch (choice) {
		...
		case 6:
			system("cls");
			printf("Таблица отличников:\n");
			printTreeTable(findBest(root));
			printf("\n\n1)=>Назад");
			getch();
			break;
		default:
