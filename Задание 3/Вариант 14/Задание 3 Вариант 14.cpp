...
struct list * bestStudents(list *lst) {
	if(!lst)
		return lst;
	if(!lst->ptr)
		if(lst->stud.history==5&&lst->stud.physics==5&&lst->stud.math==5)
			return lst;
	struct list *next=lst, *result=NULL;
	int i, length = getLength(lst);
	while(next->ptr!=NULL){
		if(next->stud.history==5&&next->stud.physics==5&&next->stud.math==5)
...
struct list * executeAction(int choice,list * lst) {
	...
	switch (choice) {
		...
		case 5:
			system("cls");
			printf("Список отличников:\n");
			listprint(bestStudents(lst));
			printf("\n\n1)=>Назад");
			getch();
			break;
		default:
