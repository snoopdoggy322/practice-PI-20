...
struct list* NotbestStudents(list* lst) {
	if (!lst)
		return lst;
	if (!lst->ptr)
		if (lst->stud.history == 3 || lst->stud.physics == 3 || lst->stud.math == 3)
			return lst;
	struct list* next = lst, * result = NULL;
	int i, length = getLength(lst);
	while (next->ptr != NULL) {
		if (next->stud.history == 3 && next->stud.physics == 3 && next->stud.math == 3)
			result = addelem(result, next->stud);
		next = next->ptr;
	}
	return result;
}
...
struct list* executeAction(int choice, list* lst) {
	...
	case 5:
		system("cls");
		printf("Список троечников:\n");
		listprint(NotbestStudents(lst));
		printf("\n\n1)=>Назад");
		getch();
		break;
	default:
